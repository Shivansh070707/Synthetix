	...
	describe('integrated actions test', () => {
		describe('when deployed', () => {
			...
			beforeEach(async () => {
				...
				hUSDContract = getContract({ target: 'ProxyhUSD', source: 'Synth' });
				...
				describe('when transferring 100k HAKA to user1', () => {
					...
					describe('when user1 issues all possible hUSD', () => {
						...
						describe('when user1 exchange 1000 hUSD for sETH (the MultiCollateralSynth)', () => {
							...
							beforeEach(async () => {
								await Synthetix.exchange(hUSD, ethers.utils.parseEther('1000'), sETH, overrides);
			...
			describe('when a pricing aggregator exists', () => {
				...
				describe('when Synthetix.anySynthOrHAKARateIsInvalid() is invoked', () => {
					it('then it returns true as expected', async () => {
						const response = await Synthetix.anySynthOrHAKARateIsInvalid();
