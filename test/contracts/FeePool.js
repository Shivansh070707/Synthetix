...
contract('FeePool', async accounts => {
	...
	describe('when the issuanceRatio is 0.2', () => {
		...
		it('should track fee withdrawals correctly', async () => {
			...
			await synthetix.exchange(hUSD, exchange, sAUD, { from: owner });
		...
		it('should correctly calculate the totalFeesAvailable for a single open period', async () => {
			...
			// Generate a fee.
			await synthetix.exchange(hUSD, amount, sAUD, { from: owner });
		...
		it('should correctly calculate the totalFeesAvailable and totalFeesBurned for multiple periods', async () => {
			...
			// Generate a fee.
			await synthetix.exchange(hUSD, amount1, sAUD, { from: owner });
			...
			// Generate a fee.
			await synthetix.exchange(hUSD, amount2, sAUD, { from: account1 });
		...
		describe('claimFees()', () => {
			describe('potential blocking conditions', () => {
				beforeEach(async () => {
					...
					await synthetix.issueSynths(toUnit('10000'), { from: owner });
					await synthetix.exchange(hUSD, toUnit('10'), sAUD, { from: owner });
					await closeFeePeriod();
				});
				['System', 'Issuance'].forEach(section => {
					describe(`when ${section} is suspended`, () => {
			...
			it('should allow a user to claim their fees in hUSD @gasprofile', async () => {
				...
				for (let i = 0; i <= length; i++) {
					...
					await synthetix.exchange(hUSD, exchange1, sAUD, { from: owner });
					await synthetix.exchange(hUSD, exchange2, sAUD, { from: account1 });
			...
			it('should allow a user to claim their fees in hUSD after burning @gasprofile', async () => {
				...
				await synthetix.exchange(hUSD, toUnit(100), sAUD, { from: account1 });
				...
				assert.bnEqual(
					await synthetix.debtBalanceOf(owner, toBytes32('hUSD')),
					toUnit('0'),
			...
			it('should allow a user to claim their fees if they minted debt during period', async () => {
				...
				const oldSynthBalance = await hUSDContract.balanceOf(owner);

				// Now we should be able to claim them.
				await feePool.claimFees({ from: owner });

				// Balance remains the same since fees are burned
				assert.bnEqual(await hUSDContract.balanceOf(owner), oldSynthBalance);
		...
		describe('claimOnBehalf', async () => {
			async function generateFees() {
				...
				// generate fee
				await synthetix.exchange(hUSD, exchange1, sAUD, { from: account1 });
			...
			it('should approve a claim on behalf for account1 by account2 and have fees in wallet', async () => {
				...
				const oldSynthBalance = await hUSDContract.balanceOf(account1);
