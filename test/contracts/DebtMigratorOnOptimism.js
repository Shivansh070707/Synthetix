...
contract('DebtMigratorOnOptimism', accounts => {
	...
	describe('when invoked by the L1 Migrator', () => {
		...
		before(async () => {
			...
			await synthetix.transfer(debtMigratorOnOptimism.address, escrowAmount.add(liquidHAKAAmount), {
				from: owner,
			});
		});
		...
		before('record balances', async () => {
			liquidHAKABalanceBefore = await synthetix.balanceOf(user);
		...
		it('updates the L2 state', async () => {
			// updates balances
			const liquidHAKABalanceAfter = await synthetix.balanceOf(user);
