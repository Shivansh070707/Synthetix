...
contract('BaseSynthetix', async accounts => {
	...
	describe('isWaitingPeriod()', () => {
		...
		describe('when a user has exchanged into sETH', () => {
			beforeEach(async () => {
				...
				await baseSynthetixImpl.exchange(hUSD, toUnit('10'), sETH, { from: owner });
			});
	...
	describe('anySynthOrHAKARateIsInvalid()', () => {
		it('should have stale rates initially', async () => {
			assert.equal(await baseSynthetixImpl.anySynthOrHAKARateIsInvalid(), true);
		});
		describe('when synth rates set', () => {
			...
			describe('when HAKA is also set', () => {
				...
				it('then no stale rates', async () => {
					assert.equal(await baseSynthetixImpl.anySynthOrHAKARateIsInvalid(), false);
				});
	...
	describe('availableCurrencyKeys()', () => {
		it('returns all currency keys by default', async () => {
			assert.deepEqual(await baseSynthetixImpl.availableCurrencyKeys(), [hUSD, sETH, sEUR, sAUD]);
		});
	});
	...
	describe('transfer()', () => {
		...
		describe('when the user has issued some hUSD and exchanged for other synths', () => {
			beforeEach(async () => {
				await baseSynthetixImpl.issueSynths(toUnit('100'), { from: owner });
				await baseSynthetixImpl.exchange(hUSD, toUnit('10'), sETH, { from: owner });
				await baseSynthetixImpl.exchange(hUSD, toUnit('10'), sAUD, { from: owner });
				await baseSynthetixImpl.exchange(hUSD, toUnit('10'), sEUR, { from: owner });
			});
			it('should transfer using the ERC20 transfer function @gasprofile', async () => {
				await baseSynthetixProxy.transfer(account1, toUnit('10'), { from: owner });
		...
		it("should lock newly received synthetix if the user's collaterisation is too high", async () => {
			...
			// Exchange into sEUR
			await baseSynthetixImpl.exchange(hUSD, maxIssuableSynths, sEUR, { from: account1 });
		...
		it('should unlock synthetix when collaterisation ratio changes', async () => {
			...
			// Exchange into sAUD
			await baseSynthetixImpl.exchange(hUSD, issuedSynths, sAUD, { from: account1 });
