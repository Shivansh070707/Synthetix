		...
		describe('when the issuanceRatio is 0.2', () => {
				...
				describe('when exchange rates set', () => {
					...
					describe('when numerous issues in many currencies', () => {
						beforeEach(async () => {
							...
							assert.bnEqual(await synthetix.totalIssuedSynths(hUSD), toUnit('0'));
							await updateDebtMonitors();
						});
				...
				describe('when exchange rates set', () => {
					...
					describe('when numerous issues in one currency', () => {
						...
						it('then totalIssuedSynths in should correctly calculate the total issued synths in hUSD', async () => {
							assert.bnEqual(await synthetix.totalIssuedSynths(hUSD), toUnit('1111'));
						});
						...
						it('and in HAKA', async () => {
							assert.bnEqual(await synthetix.totalIssuedSynths(HAKA), divideDecimal('1111', '2'));
						});
					...
					describe('when numerous issues in many currencies', () => {
						...
						it('then totalIssuedSynths in should correctly calculate the total issued synths in hUSD', async () => {
							assert.bnEqual(await synthetix.totalIssuedSynths(hUSD), toUnit('2200'));
						});
						it('and in another synth currency', async () => {
							assert.bnEqual(await synthetix.totalIssuedSynths(sAUD), toUnit('4400', '2'));
						});
						it('and in HAKA', async () => {
							assert.bnEqual(await synthetix.totalIssuedSynths(HAKA), divideDecimal('2200', '2'));
						});
						it('and in a non-synth currency', async () => {
			...
			describe('debtBalance()', () => {
				it('should not change debt balance % if exchange rates change', async () => {
					...
					await synthetix.exchange(hUSD, amountIssuedAcc2, sAUD, { from: account2 });

					const PRECISE_UNIT = web3.utils.toWei(web3.utils.toBN('1'), 'gether');
					let totalIssuedSynthhUSD = await synthetix.totalIssuedSynths(hUSD);
					const account1DebtRatio = divideDecimal(
						amountIssuedAcc1,
						totalIssuedSynthhUSD,
						PRECISE_UNIT
					);
					...
					totalIssuedSynthhUSD = await synthetix.totalIssuedSynths(hUSD);
					const conversionFactor = web3.utils.toBN(1000000000);
					const expectedDebtAccount1 = multiplyDecimal(
						account1DebtRatio,
						totalIssuedSynthhUSD.mul(conversionFactor),
						PRECISE_UNIT
					...
					assert.bnClose(await synthetix.debtBalanceOf(account1, hUSD), expectedDebtAccount1);
					assert.bnClose(await synthetix.debtBalanceOf(account2, hUSD), expectedDebtAccount2);
				});
				...
				it("should correctly calculate a user's debt balance without prior issuance", async () => {
					...
					const debt1 = await synthetix.debtBalanceOf(account1, toBytes32('hUSD'));
					const debt2 = await synthetix.debtBalanceOf(account2, toBytes32('hUSD'));
				...
				it("should correctly calculate a user's debt balance with prior issuance", async () => {
					...
					const debt = await synthetix.debtBalanceOf(account1, toBytes32('hUSD'));
			...
			describe('adding and removing synths', () => {
				...
				describe('when another synth is added with 0 supply', () => {
					...
					it('should be able to query multiple synth addresses', async () => {
						...
						assert.equal(synthAddresses[2], hUSDContract.address);
						...
						describe('when the synth has a rate', () => {
							...
							describe('when another user exchanges into the synth', () => {
								beforeEach(async () => {
									...
									await synthetix.exchange(hUSD, toUnit('100'), currencyKey, { from: account2 });
								});
								...
								describe('when the same user exchanges out of the synth', () => {
									beforeEach(async () => {
										...
										await synthetix.exchange(currencyKey, toUnit('1'), hUSD, { from: account2 });
									});
									describe('when the synth is removed', () => {
										beforeEach(async () => {
											await issuer.removeSynth(currencyKey, { from: owner });
							...
							describe('when a debt snapshot is taken', () => {
								...
								describe('when the synth is removed', () => {
									...
									beforeEach(async () => {
										// base conditions
										assert.equal(await hUSDContract.balanceOf(synthRedeemer.address), '0');
									...
									it('issues the equivalent amount of hUSD', async () => {
										const amountOfhUSDIssued = await hUSDContract.balanceOf(synthRedeemer.address);
			...
			describe('issuance', () => {
				...
				it('should allow an issuer to issue synths in one flavour', async () => {
					...
					assert.bnEqual(await synthetix.totalIssuedSynths(hUSD), toUnit('10'));

					// And account1 should own 100% of the debt.
					assert.bnEqual(await synthetix.totalIssuedSynths(hUSD), toUnit('10'));
					assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('10'));
				});
				...
				it('should allow two issuers to issue synths in one flavour', async () => {
					...
					assert.bnEqual(await synthetix.totalIssuedSynths(hUSD), toUnit('30'));
					...
					assert.bnClose(await synthetix.debtBalanceOf(account1, hUSD), toUnit('10'));
					assert.bnClose(await synthetix.debtBalanceOf(account2, hUSD), toUnit('20'));
				});
				...
				it('should allow multi-issuance in one flavour', async () => {
					...
					assert.bnEqual(await synthetix.totalIssuedSynths(hUSD), toUnit('40'));

					// And the debt should be split 50/50.
					// But there's a small rounding error.
					// This is ok, as when the last person exits the system, their debt percentage is always 100% so
					// these rounding errors don't cause the system to be out of balance.
					assert.bnClose(await synthetix.debtBalanceOf(account1, hUSD), toUnit('20'));
					assert.bnClose(await synthetix.debtBalanceOf(account2, hUSD), toUnit('20'));
				});
				...
				describe('issueMaxSynths', () => {
					it('should allow an issuer to issue max synths in one flavour', async () => {
						...
						assert.bnEqual(await synthetix.totalIssuedSynths(hUSD), toUnit('200'));

						// And account1 should own all of it.
						assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('200'));
					});
				});
			...
			describe('burning', () => {
				...
				it('should allow an issuer with outstanding debt to burn synths and decrease debt', async () => {
					...
					assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('100'));
				});
				...
				it('should revert when trying to burn synths that do not exist', async () => {
					...
					const debtBefore = await synthetix.debtBalanceOf(account1, hUSD);
				...
				it("should successfully burn all user's synths @gasprofile", async () => {
					...
					// Then try to burn them all. Only 10 synths (and fees) should be gone.
					await synthetix.burnSynths(await hUSDContract.balanceOf(account1), {
						from: account1,
					});
				...
				it('should burn the correct amount of synths', async () => {
					...
					const debt = await synthetix.debtBalanceOf(account1, hUSD);
				...
				describe('debt calculation in multi-issuance scenarios', () => {
					it('should correctly calculate debt in a multi-issuance multi-burn scenario @gasprofile', async () => {
						...
						const debtBalance1After = await synthetix.debtBalanceOf(account1, hUSD);
						const debtBalance2After = await synthetix.debtBalanceOf(account2, hUSD);
						const debtBalance3After = await synthetix.debtBalanceOf(account3, hUSD);
					...
					it('should allow user to burn all synths issued even after other users have issued', async () => {
						...
						const debtBalanceBefore = await synthetix.debtBalanceOf(account1, hUSD);
						await synthetix.burnSynths(debtBalanceBefore, { from: account1 });
						const debtBalanceAfter = await synthetix.debtBalanceOf(account1, hUSD);
					...
					it('should allow users to burn their debt and adjust the debtBalanceOf correctly for remaining users', async () => {
						...
						let debtBalance1After = await synthetix.debtBalanceOf(account1, hUSD);
						let debtBalance2After = await synthetix.debtBalanceOf(account2, hUSD);
						...
						debtBalance1After = await synthetix.debtBalanceOf(account1, hUSD);
						debtBalance2After = await synthetix.debtBalanceOf(account2, hUSD);
				...
				describe('burnSynthsToTarget', () => {
					beforeEach(async () => {
						...
						// Issue
						await synthetix.issueMaxSynths({ from: account1 });
						assert.bnClose(await synthetix.debtBalanceOf(account1, hUSD), toUnit('8000'));
					...
					describe('when the HAKA price drops 50%', () => {
						...
						it('then calling burnSynthsToTarget() reduces hUSD to c-ratio target', async () => {
							await synthetix.burnSynthsToTarget({ from: account1 });
							assert.bnClose(await synthetix.debtBalanceOf(account1, hUSD), toUnit('4000'));
						});
					...
					describe('when the HAKA price drops 10%', () => {
						...
						it('then calling burnSynthsToTarget() reduces hUSD to c-ratio target', async () => {
							await synthetix.burnSynthsToTarget({ from: account1 });
							assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('7200'));
						});
						it('then fees are claimable', async () => {
							await synthetix.burnSynthsToTarget({ from: account1 });
							assert.equal(await feePool.isFeesClaimable(account1), true);
					...
					describe('when the HAKA price drops 90%', () => {
						...
						it('then calling burnSynthsToTarget() reduces hUSD to c-ratio target', async () => {
							await synthetix.burnSynthsToTarget({ from: account1 });
							assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('800'));
						});
						it('then fees are claimable', async () => {
							await synthetix.burnSynthsToTarget({ from: account1 });
							assert.equal(await feePool.isFeesClaimable(account1), true);
					...
					describe('given the waiting period is set to 60s', () => {
						...
						describe('and a user has 1250 hUSD issued', () => {
							...
							describe('and is has been exchanged into sEUR at a rate of 1.25:1 and the waiting period has expired', () => {
								beforeEach(async () => {
									await synthetix.exchange(hUSD, amount, sEUR, { from: account1 });
								...
								describe('and they have exchanged all of it back into hUSD', () => {
									beforeEach(async () => {
										await synthetix.exchange(sEUR, toUnit('1000'), hUSD, { from: account1 });
									});
										...
										describe('when they attempt to burn the hUSD', () => {
											...
											it('then it succeeds and burns the entire hUSD amount', async () => {
												const logs = await getDecodedLogs({
													hash: txn.tx,
													contracts: [synthetix, hUSDContract],
												});
												...
												const debtBalance = await synthetix.debtBalanceOf(account1, hUSD);
			...
			describe('debt calculation in multi-issuance scenarios', () => {
				...
				it("should allow me to burn all synths I've issued when there are other issuers", async () => {
					...
					await synthetix.issueMaxSynths({ from: account1 });
					const debtBalance1 = await synthetix.debtBalanceOf(account1, hUSD);
					...
					let debt = await synthetix.debtBalanceOf(account2, hUSD);
					...
					debt = await synthetix.debtBalanceOf(account2, hUSD);
			...
			describe('multiple issue and burn scenarios', () => {
				it('should correctly calculate debt in a high issuance and burn scenario', async () => {
					...
					for (let i = 0; i < totalTimesToIssue; i++) {
						...
						// const db = await synthetix.debtBalanceOf(account2, hUSD);
					...
					const debtBalance = await synthetix.debtBalanceOf(account2, hUSD);
				...
				it('should correctly calculate debt in a high volume contrast issuance and burn scenario', async () => {
					...
					const debtBalance2 = await synthetix.debtBalanceOf(account2, hUSD);
			...
			it("should prevent more issuance if the user's collaterisation changes to be insufficient", async () => {
				...
				// exchange into sEUR
				await synthetix.exchange(hUSD, issuedSynths, sEUR, { from: account1 });
			...
			describe('check collaterisation ratio', () => {
				...
				it("should not include escrowed synthetix when calculating a user's collaterisation ratio", async () => {
					...
					const expectedCollaterisationRatio = divideDecimal(
						maxIssuable,
						multiplyDecimal(transferredSynthetixs, haka2usdRate)
					);
				...
				it("should include escrowed reward synthetix when calculating a user's collateralisation ratio", async () => {
					...
					const expectedCollaterisationRatio = divideDecimal(
						maxIssuable,
						multiplyDecimal(escrowedSynthetixs.add(transferredSynthetixs), haka2usdRate)
					);
					assert.bnEqual(collaterisationRatio, expectedCollaterisationRatio);
				});
				...
				it('should permit user to issue hUSD debt with only escrowed HAKA as collateral (no HAKA in wallet)', async () => {
					...
					const hakaBalance = await synthetix.balanceOf(account1);
					...
					assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('300'));
				});
			...
			describe('issue and burn on behalf', async () => {
				...
				it('should approveIssueOnBehalf and IssueMaxSynths', async () => {
					...
					assert.bnEqual(hUSDBalanceAfter, hUSDBalanceBefore.add(issuableSynths));
				});
				...
				it('should approveBurnOnBehalf and BurnSynths', async () => {
					...
					const hUSDBalanceBefore = await hUSDContract.balanceOf(account1);
					await synthetix.burnSynthsOnBehalf(authoriser, hUSDBalanceBefore, { from: delegate });
			...
			describe('when Wrapper is set', async () => {
				it('should have zero totalIssuedSynths', async () => {
					assert.bnEqual(
						await synthetix.totalIssuedSynths(hUSD),
						await synthetix.totalIssuedSynthsExcludeOtherCollateral(hUSD)
					);
				});
			...
			describe('debt shares integration', async () => {
				...
				it('burns the correct number of debt shares', async () => {
					...
					assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('270'));
				});
				...
				describe('when debt ratio changes', () => {
					...
					it('has adjusted debt', async () => {
						assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('450')); // = 750 sds * 0.6
					});
					...
					it('mints at adjusted rate', async () => {
						...
						assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('750')); // = 450 (hUSD from before ) + 300
					});
				});
				...
				describe('issued synths aggregator', async () => {
					...
					it('has no effect on mint or burn', async () => {
						...
						assert.bnEqual(await synthetix.debtBalanceOf(account1, hUSD), toUnit('270')); // 300 - 30 husd
					});
				});
			});
