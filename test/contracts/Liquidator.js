...
contract('Liquidator', accounts => {
	...
	const setLiquidHAKABalance = async (account, amount) => {
		// burn debt
		await synthetix.burnSynths(await synthhUSD.balanceOf(account), { from: account });
		// remove all haka
			...
			describe('when Alice is undercollateralized', () => {
				...
				describe('given Alice has $600 Debt, $800 worth of HAKA Collateral and c-ratio at 133.33%', () => {
					describe('when Alice calls self liquidate', () => {
						...
						beforeEach(async () => {
							...
							aliceDebtValueBefore = await synthetix.debtBalanceOf(alice, hUSD);

							// Record Bobs state
							bobDebtValueBefore = await synthetix.debtBalanceOf(bob, hUSD);
						...
						it('it succeeds and the ratio is fixed', async () => {
							...
							assert.eventEqual(
								...
								{
									account: alice,
									hakaRedeemed: await synthetix.balanceOf(liquidatorRewards.address),
								}
							);

							// Make sure the other staker, Bob, gets the redeemed HAKA bonus.
							const bobDebtValueAfter = await synthetix.debtBalanceOf(bob, hUSD);
				...
				describe('with some HAKA in escrow', () => {
					...
					it('escrow balance is not used for self-liquidation', async () => {
						const hakaBalanceBefore = await synthetix.balanceOf(alice);
						const debtBefore = await synthetix.debtBalanceOf(alice, hUSD);
						const totalDebt = await synthetix.totalIssuedSynths(hUSD);
						// just above the liquidation ratio
						await updateHAKAPrice('1');
						await synthetix.liquidateSelf({ from: alice });
						// liquid haka is reduced
						const hakaBalanceAfter = await synthetix.balanceOf(alice);
						assert.bnLt(hakaBalanceAfter, hakaBalanceBefore);
						// escrow untouched
						assert.bnEqual(await rewardEscrowV2.balanceOf(alice), escrowBalance);
						// system debt is the same
						assert.bnEqual(await synthetix.totalIssuedSynths(hUSD), totalDebt);
						...
						assert.bnClose(
							await synthetix.debtBalanceOf(alice, hUSD),
				...
				describe('with only escrowed HAKA', () => {
					...
					it('should revert with cannot self liquidate', async () => {
						...
						const hakaBalance = await synthetix.balanceOf(alice);
						...
						describe('when Alice does not fix her c-ratio ', () => {
							...
							describe('given Alice has $600 Debt, $800 worth of HAKA Collateral and c-ratio at 133.33%', () => {
								describe('when bob calls liquidateDelinquentAccount on Alice', () => {
									...
									beforeEach(async () => {
										...
										// Record Bobs state
										bobHakaBalanceBefore = await synthetix.balanceOf(bob);
							...
							describe('with only escrowed HAKA', () => {
								...
								it('escrow balance is used for liquidation (partial)', async () => {
									...
									const debtAfter = await synthetix.debtBalanceOf(alice, hUSD);
