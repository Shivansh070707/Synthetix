...
contract('Rewards Integration Tests', accounts => {
	...
	beforeEach(async () => {
		...
		const hakaTotalSupply = await synthetix.totalSupply();
		const thirdOfHAKA = third(hakaTotalSupply);

		await synthetix.transfer(account1, thirdOfHAKA, { from: owner });
		await synthetix.transfer(account2, thirdOfHAKA, { from: owner });
		await synthetix.transfer(account3, thirdOfHAKA, { from: owner });
	...
	describe('Exchange Rate Shift tests', async () => {
		it('should assign accounts (1,2,3) to have (40%,40%,20%) of the debt/rewards', async () => {
			...
			await synthetix.exchange(hUSD, tenK, sBTC, { from: account1 });
			await synthetix.exchange(hUSD, tenK, sBTC, { from: account2 });
			...
			await synthetix.exchange(sBTC, amountReceived, hUSD, { from: account1 });
			await synthetix.exchange(sBTC, amountReceived, hUSD, { from: account2 });
			...
			// await synthetix.exchange(sBTC, acc1sBTCBalance, hUSD, { from: account1 });
	...
	describe('When user is the last to call claimFees()', () => {
		beforeEach(async () => {
			...
			await synthetix.exchange(hUSD, oneThousand, sAUD, { from: account2 });
			await synthetix.exchange(hUSD, oneThousand, sAUD, { from: account1 });
