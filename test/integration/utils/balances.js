...
async function _gethUSD({ ctx, user, amount }) {
	const { ProxySynthetix, ProxyhUSD } = ctx.contracts;
	let { Synthetix, SynthhUSD } = ctx.contracts;

	// connect via proxy
	Synthetix = new ethers.Contract(ProxySynthetix.address, Synthetix.interface, ctx.provider);
	SynthhUSD = new ethers.Contract(ProxyhUSD.address, SynthhUSD.interface, ctx.provider);
	...
	const availableOwnerHAKA = await Synthetix.transferableSynthetix(ctx.users.owner.address);
	if (availableOwnerHAKA.lt(requiredHAKA.mul(2))) {
		await _getHAKAForOwner({ ctx, amount: requiredHAKA.mul(2).sub(availableOwnerHAKA) });
	}

	tx = await Synthetix.transfer(tmpWallet.address, requiredHAKA.mul(2));
	await tx.wait();

	tx = await Synthetix.connect(tmpWallet).issueSynths(amount);
	await tx.wait();

	tx = await SynthhUSD.connect(tmpWallet).transfer(user.address, amount);
	await tx.wait();
}
