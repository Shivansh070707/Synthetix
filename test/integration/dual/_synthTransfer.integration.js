...
describe('initiateSynthTransfer() integration tests (L1, L2)', () => {
	...
	let SynthhUSD, SynthsETH, SynthetixBridgeToOptimism, SystemSettings;

	let SynthhUSDL2, SynthsETHL2, SynthetixBridgeToBase, SystemSettingsL2, SystemStatusL2;
	...
	describe('when the owner sends hUSD and sETH', () => {
		before('target contracts and users', () => {
			({ SynthhUSD, SynthsETH, SynthetixBridgeToOptimism, SystemSettings } = ctx.l1.contracts);
			({
				SynthhUSD: SynthhUSDL2,
		...
		before('record balances', async () => {
			userBalance = await SynthhUSD.balanceOf(user.address);
			userL2Balance = await SynthhUSDL2.balanceOf(user.address);
		});
		...
		it('decreases the owner balance', async () => {
			const newOwnerBalance = await SynthhUSD.balanceOf(user.address);
		...
		describe('when the deposit gets picked up in L2', () => {
			...
			it('increases the owner balance', async () => {
				assert.bnEqual(
					await SynthhUSDL2.balanceOf(user.address),
			...
			describe('send back to L1', () => {
				...
				it('decreases the owner balance', async () => {
					const newBalance = await SynthhUSDL2.balanceOf(user.address);
				...
				describe('picked up on L1', () => {
					...
					it('increases the owner balance', async () => {
						assert.bnEqual(await SynthhUSD.balanceOf(user.address), userBalance);
					});
				});
			});
		});
