...
function itCanLiquidate({ ctx }) {
	describe('liquidating', () => {
			...
			describe('when the liquidation delay passes', () => {
				...
				describe('getting liquidated', () => {
					...
					before('liquidatorUser calls liquidateDelinquentAccount', async () => {
						...
						beforeFlagRewardCredittedHaka = await Synthetix.balanceOf(flaggerUser.address);
						beforeLiquidateRewardCredittedHaka = await Synthetix.balanceOf(liquidatorUser.address);
						beforeRemainingRewardCredittedHaka = await Synthetix.balanceOf(
			...
			describe('when the liquidation delay passes', () => {
				...
				describe('getting liquidated', () => {
					...
					before('liquidatorUser calls liquidateDelinquentAccount', async () => {
						...
						beforeDebtBalance = await Synthetix.debtBalanceOf(user7.address, toBytes32('hUSD'));
		...
		describe('full liquidation with a majority of collateral in escrow', () => {
			...
			before('liquidatorUser calls liquidateDelinquentAccount', async () => {
				beforeHakaBalance = await Synthetix.balanceOf(user8.address);
				...
				beforeSharesSupply = await SynthetixDebtShare.totalSupply();
				beforeDebtBalance = await Synthetix.debtBalanceOf(user8.address, toBytes32('hUSD'));
				beforeRewardsCredittedHaka = await Synthetix.balanceOf(LiquidatorRewards.address);
			...
			it('should remove all transferable collateral', async () => {
				const afterHakaBalance = await Synthetix.balanceOf(user8.address);
			...
			it('should remove all debt', async () => {
				const afterDebtBalance = await Synthetix.debtBalanceOf(user8.address, toBytes32('hUSD'));
