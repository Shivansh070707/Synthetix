	...
	describe('opening positions', function() {
		...
		describe('position management', () => {
			...
			it('user can transferMargin and withdraw it', async () => {
				...
				const balance = await SynthhUSD.balanceOf(someUser.address);
				// transfer
				await market.transferMargin(margin);
				assert.bnEqual(await SynthhUSD.balanceOf(someUser.address), balance.sub(margin));

				// withdraw
				await (await market.withdrawAllMargin()).wait();
				const withdrawBalance = await SynthhUSD.balanceOf(someUser.address);
