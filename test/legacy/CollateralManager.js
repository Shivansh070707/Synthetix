...
contract('CollateralManager', async accounts => {
	...
	const setupManager = async () => {
		...
		await ceth.addSynths(
			['SynthhUSD', 'SynthsETH'].map(toBytes32),
			['hUSD', 'sETH'].map(toBytes32),
			{ from: owner }
		);
		await cerc20.addSynths(
			['SynthhUSD', 'SynthsBTC'].map(toBytes32),
	...
	beforeEach(async () => {
		...
		await issue(hUSDSynth, toUnit(1000), owner);
	...
	describe('adding synths', async () => {
		...
		it('should not allow duplicate synths to be added', async () => {
			await manager.addSynths([toBytes32('SynthhUSD')], [toBytes32('hUSD')], {
				from: owner,
			});
			assert.isTrue(
				await manager.areSynthsAndCurrenciesSet(
		...
		it('should revert when input array lengths dont match', async () => {
			await assert.revert(
				manager.addSynths([toBytes32('SynthhUSD'), toBytes32('SynthsBTC')], [toBytes32('hUSD')], {
					from: owner,
				}),
		...
		it('should revert when input array lengths dont match', async () => {
			await assert.revert(
				manager.removeSynths(
					[toBytes32('SynthhUSD'), toBytes32('SynthsBTC')],
					[toBytes32('hUSD')],
					{
						from: owner,
					}
	...
	describe('should only allow opening positions up to the debt limiit', async () => {
		beforeEach(async () => {
			await issue(hUSDSynth, toUnit(15000000), account1);
			await hUSDSynth.approve(short.address, toUnit(15000000), { from: account1 });
		});
	...
	describe('tracking synth balances across collaterals', async () => {
		...
		it('should reduce the hUSD balance when a loan is closed', async () => {
			issue(hUSDSynth, toUnit(10), account1);
