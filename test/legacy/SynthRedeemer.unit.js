	...
	describe('when a contract is instantiated', () => {
		...
		beforeEach(async () => {
			({ mocks: this.mocks, resolver: this.resolver } = await prepareSmocks({
				contracts: ['Issuer', 'Synth:SynthhUSD'],
		...
		describe('deprecate()', () => {
			...
			describe('when the synth has some supply', () => {
				...
				describe('when there is sufficient hUSD for the synth to be deprecated', () => {
					beforeEach(async () => {
						...
						this.mocks['SynthhUSD'].balanceOf.returns(parseEther('10000'));
					});
			...
			describe('when the synth has some supply', () => {
				...
				describe('when there is sufficient hUSD for the synth to be deprecated', () => {
					beforeEach(async () => {
						// smock hUSD balance to prevent the deprecation failing
						this.mocks['SynthhUSD'].balanceOf.returns(parseEther('2000'));
					});
					...
					describe('when the user has a synth balance', () => {
						...
						describe('when redemption is called by the user', () => {
							...
							it('transfers the correct amount of hUSD to the user', async () => {
								assert.equal(this.mocks['SynthhUSD'].transfer.calls.length, 1);
								assert.equal(this.mocks['SynthhUSD'].transfer.calls[0][0], account1);
								assert.bnEqual(
									this.mocks['SynthhUSD'].transfer.calls[0][1],
								...
								describe('when redemption is called by the user', () => {
									...
									[0, 1].forEach(i => {
										describe(`For synth ${i}`, () => {
											...
											it('transfers the correct amount of hUSD to the user', async () => {
												assert.equal(this.mocks['SynthhUSD'].transfer.calls.length, 2);
												assert.equal(this.mocks['SynthhUSD'].transfer.calls[i][0], account1);
												assert.bnEqual(
													this.mocks['SynthhUSD'].transfer.calls[i][1],
													parseEther('10') // 5 units deprecated at price 2 is 10
												);
											});
										});
