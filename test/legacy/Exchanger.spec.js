...
contract('Exchanger (spec tests)', async accounts => {
		...
		describe('waitingPeriodSecs', () => {
			...
			describe('given it is configured to 90', () => {
				...
				describe('and there is an exchange', () => {
					beforeEach(async () => {
						await synthetix.exchange(hUSD, toUnit('100'), sEUR, { from: account1 });
					});
		...
		describe('When the waiting period is set to 0', () => {
			...
			describe('When exchanging', () => {
				...
				beforeEach(async () => {
					...
					await synthetix.exchange(hUSD, toUnit('10'), sETH, { from: owner });
				});
				...
				it('can exchange back without waiting', async () => {
					...
					await synthetix.exchange(sETH, amountReceived, hUSD, { from: owner });
				...
				describe('When the waiting period is switched on again', () => {
					...
					describe('a new exchange takes place', () => {
						...
						beforeEach(async () => {
							...
							exchangeTransaction = await synthetix.exchange(hUSD, amountOfSrcExchanged, sETH, {
								from: owner,
							});
						});
						...
						describe('When the waiting period is set back to 0', () => {
							...
							describe('new trades take place', () => {
								beforeEach(async () => {
									...
									await synthetix.exchange(sETH, sEthBalance, hUSD, { from: owner });
									await synthetix.exchange(hUSD, toUnit('10'), sEUR, { from: owner });
								});
		...
		describe('priceDeviationThresholdFactor()', () => {
			...
			describe('when a user exchanges into sETH over the default threshold factor', () => {
				...
				beforeEach(async () => {
					...
					const { tx: hash } = await synthetix.exchange(hUSD, toUnit('1'), sETH, {
						from: account1,
					});
				...
				describe('when the factor is set to 3.1', () => {
					...
					describe('when a user exchanges into sETH over the default threshold factor, but under the new one', () => {
						beforeEach(async () => {
							...
							// base rate of sETH is 100 from shared setup above
							await updateRates([sETH], [toUnit('300')]);
							await synthetix.exchange(hUSD, toUnit('1'), sETH, { from: account1 });
						});
				...
				describe('when a user with hUSD has performed an exchange into sEUR', () => {
					...
					describe('when 55 seconds has elapsed', () => {
						...
						describe('when another user does the same exchange', () => {
							beforeEach(async () => {
								await synthetix.exchange(hUSD, toUnit('100'), sEUR, { from: account2 });
							});
		...
		describe('given exchange fee rates are configured into categories', () => {
			...
			describe('when calling getAmountsForExchange', () => {
				describe('and the destination is a crypto synth', () => {
					...
					let feeRate;
					beforeEach(async () => {
						await synthetix.exchange(hUSD, amountIssued, sBTC, { from: account1 });
						const {
							amountReceived,
							fee,
							exchangeFeeRate,
				...
				describe('and the destination is a fiat synth', () => {
					...
					let feeRate;
					beforeEach(async () => {
						await synthetix.exchange(hUSD, amountIssued, sEUR, { from: account1 });
						const {
							amountReceived,
							fee,
							exchangeFeeRate,
				...
				describe('dynamic fee when rates change', () => {
					...
					it('no exchange happens when dynamic fee is too high', async () => {
						...
						async function echangeSuccessful() {
							// this should work
							const txn = await synthetix.exchange(sETH, toUnit('1'), hUSD, { from: account1 });
							const logs = await getDecodedLogs({
								hash: txn.tx,
								contracts: [synthetix, exchanger, systemStatus],
							});
										...
										describe('when the price halves for hUSD:sEUR to 1:1', () => {
											...
											describe('when the user makes a 2nd exchange of 100 hUSD into hUSD:sEUR at 1:1', () => {
												beforeEach(async () => {
													...
													await synthetix.exchange(hUSD, amountOfSrcExchanged, sEUR, {
														from: account1,
													});
												});
												...
												describe('when the price gains for sBTC more than the loss of the sEUR change', () => {
													...
													describe('when the same user exchanges some hUSD into sBTC - the same destination', () => {
														...
														beforeEach(async () => {
															...
															await synthetix.exchange(hUSD, amountOfSrcExchangedSecondary, sBTC, {
																from: account1,
															});
														});
				...
				['System', 'Exchange', 'SynthExchange', 'Synth'].forEach(section => {
					describe(`when ${section} is suspended`, () => {
						...
						it('then calling exchange() reverts', async () => {
							await assert.revert(
								synthetix.exchange(hUSD, toUnit('1'), sETH, { from: account1 }),
								'Operation prohibited'
							);
						});
						describe(`when ${section} is resumed`, () => {
			...
			describe('various exchange scenarios', () => {
				describe('when a user has 1000 hUSD', () => {
					...
					it('should allow a user to exchange the synths they hold in one flavour for another', async () => {
						...
						await synthetix.exchange(hUSD, amountIssued, sAUD, { from: account1 });
					...
					it('should emit a SynthExchange event @gasprofile', async () => {
						// Exchange hUSD to sAUD
						const txn = await synthetix.exchange(hUSD, amountIssued, sAUD, {
							from: account1,
						});
					...
					it('when a user tries to exchange more than they have, then it fails', async () => {
						await assert.revert(
							synthetix.exchange(sAUD, toUnit('1'), hUSD, {
								from: account1,
							}),
					...
					it('when a user tries to exchange more than they have, then it fails', async () => {
						await assert.revert(
							synthetix.exchange(hUSD, toUnit('1001'), sAUD, {
								from: account1,
							}),
							'SafeMath: subtraction overflow'
						);
					...
					describe('exchanging on behalf', async () => {
						...
						describe('when not approved it should revert on', async () => {
							it('exchangeOnBehalf', async () => {
								await assert.revert(
									synthetix.exchangeOnBehalf(authoriser, sAUD, toUnit('1'), hUSD, {
										from: delegate,
									}),
							...
							describe('suspension conditions on Synthetix.exchangeOnBehalf()', () => {
								...
								['System', 'Exchange', 'SynthExchange', 'Synth'].forEach(section => {
									describe(`when ${section} is suspended`, () => {
										...
										it('then calling exchange() reverts', async () => {
											await assert.revert(
												synthetix.exchangeOnBehalf(authoriser, hUSD, amountIssued, sAUD, {
													from: delegate,
												}),
												'Operation prohibited'
											);
										...
										describe(`when ${section} is resumed`, () => {
											...
											it('then calling exchange() succeeds', async () => {
												await synthetix.exchangeOnBehalf(authoriser, hUSD, amountIssued, sAUD, {
													from: delegate,
												});
											});
										});
				...
				describe('when an aggregator is added to the exchangeRates', () => {
					...
					describe('when exchanging out of that synth', () => {
						...
						it('then it causes a breaker trip from price deviation', async () => {
							// await assert.revert(
							const { tx: hash } = await synthetix.exchange(sETH, toUnit('1'), hUSD, {
								from: account1,
							});
	...
	const itFailsToExchangeWithVirtual = () => {
		describe('it cannot use exchangeWithVirtual()', () => {
			it('errors with not implemented when attempted to exchange', async () => {
				await assert.revert(
					synthetix.exchangeWithVirtual(hUSD, amountIssued, sAUD, toBytes32(), {
						from: account1,
					}),
				...
				describe('when the necessary configuration been set', () => {
					...
					describe('when a user sets a minimum amount', () => {
						...
						it('reverts when the received amount is too low', async () => {
							await assert.revert(
								synthetix.exchangeAtomically(hUSD, amountIn, sETH, toBytes32(), toUnit('.498'), {
									from: account1,
								}),
					...
					describe('when a fee override has been set for atomic exchanges', () => {
						...
						beforeEach(async () => {
							await synthetix.exchangeAtomically(hUSD, amountIn, sETH, toBytes32(), 0, {
								from: account1,
							});
					...
					describe('when a user exchanges without a tracking code', () => {
						let txn;
						beforeEach(async () => {
							txn = await synthetix.exchangeAtomically(hUSD, toUnit('10'), sETH, toBytes32(), 0, {
								from: account1,
							});
						});
						it('then no ExchangeTracking is emitted (as no tracking code supplied)', async () => {
	...
	const itFailsToExchangeAtomically = () => {
		describe('it cannot exchange atomically', () => {
			it('errors with not implemented when attempted to exchange', async () => {
				await assert.revert(
					synthetix.exchangeAtomically(hUSD, amountIssued, sETH, toBytes32(), 0, {
						from: account1,
					}),
					'Cannot be run on this layer'
				);
					...
					describe('suspension is triggered via exchanging', () => {
						describe('given the user has some sETH', () => {
							...
							describe('with a prior exchange from another user into the source', () => {
								beforeEach(async () => {
									await synthetix.exchange(hUSD, toUnit('1'), sETH, { from: account2 });
								});
										...
										describe('and the user makes another exchange into sETH', () => {
											...
											describe('and the sETH rate goes back up 25% (from 80 to 100)', () => {
												...
												describe('and the waiting period expires', () => {
													...
													describe('and the user makes another exchange into sETH', () => {
														beforeEach(async () => {
															await synthetix.exchange(hUSD, toUnit('100'), sETH, {
																from: account1,
															});
														});
								...
								describe('and the aggregator has a rate (so the exchange succeeds)', () => {
									...
									describe('when a user exchanges out of the aggregated rate into hUSD', () => {
										beforeEach(async () => {
											// give the user some sETH
											await sETHContract.issue(account1, toUnit('1'));
											await synthetix.exchange(sETH, toUnit('1'), hUSD, { from: account1 });
										});
