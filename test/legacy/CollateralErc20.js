...
contract('CollateralErc20', async accounts => {
	...
	const issuehUSDToAccount = async (issueAmount, receiver) => {
		...
		await hUSDSynth.issue(receiver, issueAmount, {
			from: owner,
		});
	};
	...
	it('should set constructor params on deployment', async () => {
		...
		assert.equal(await cerc20.synths(0), toBytes32('SynthhUSD'));
	...
	describe('opening', async () => {
		...
		describe('should open a btc loan denominated in hUSD', async () => {
			...
			it('should issue the correct amount to the borrower', async () => {
				...
				assert.bnEqual(await hUSDSynth.balanceOf(account1), expectedBal);
			});

			it('should issue the minting fee to the fee pool', async () => {
				const feePoolBalance = await hUSDSynth.balanceOf(FEE_ADDRESS);
	...
	describe('repayments', async () => {
		...
		describe('should allow repayments on an hUSD loan', async () => {
			...
			it('should work reduce the repayers balance', async () => {
				...
				assert.bnEqual(await hUSDSynth.balanceOf(account2), expectedBalance);
			});
	...
	describe('liquidations', async () => {
		...
		describe('should allow liquidations on an undercollateralised hUSD loan', async () => {
			...
			it('should reduce the liquidators synth amount', async () => {
				const liquidatorBalance = await hUSDSynth.balanceOf(account2);
			...
			it('should pay the interest to the fee pool', async () => {
				const balance = await hUSDSynth.balanceOf(FEE_ADDRESS);
