...
contract('CollateralShort', async accounts => {
	...
	const setupShort = async () => {
		...
		await manager.addSynths(
			[toBytes32('SynthhUSD'), toBytes32('SynthsBTC'), toBytes32('SynthsETH')],
		...
		assert.isTrue(
			await manager.areSynthsAndCurrenciesSet(
				['SynthhUSD', 'SynthsBTC', 'SynthsETH'].map(toBytes32),
		...
		await hUSDSynth.approve(short.address, toUnit(100000), { from: account1 });
	};
	...
	before(async () => {
		...
		await issue(hUSDSynth, toUnit(100000), owner);
	...
	describe('logic', () => {
		...
		it('should access its dependencies via the address resolver', async () => {
			assert.equal(await addressResolver.getAddress(toBytes32('SynthhUSD')), hUSDSynth.address);
		...
		describe('opening shorts', async () => {
			describe('should open a btc short', async () => {
				...
				beforeEach(async () => {
					await issue(hUSDSynth, husdCollateral, account1);

					tx = await short.open(husdCollateral, oneBTC, sBTC, { from: account1 });
				...
				it('should correclty issue the right balance to the shorter', async () => {
					...
					assert.bnEqual(await hUSDSynth.balanceOf(account1), hUSDProceeds);
				});
				...
				it('should transfer the hUSD to the contract', async () => {
					assert.bnEqual(await hUSDSynth.balanceOf(short.address), husdCollateral);
				});
			});
			...
			describe('should open an eth short', async () => {
				...
				beforeEach(async () => {
					...
					tx = await short.open(husdCollateral, oneETH, sETH, { from: account1 });
		...
		describe('Repaying shorts', async () => {
			...
			const accrueInterest = async () => {
				...
				tx = await short.open(husdCollateral, ethAmountToShort, sETH, { from: account1 });
			...
			beforeEach(async () => {
				...
				beforeFeePoolBalance = await hUSDSynth.balanceOf(FEE_ADDRESS);
				beforeShortBalance = await hUSDSynth.balanceOf(short.address);
				beforeUserBalance = await hUSDSynth.balanceOf(account1);
			...
			it('should repay with collateral and update the loan', async () => {
				...
				assert.deepEqual(
					await hUSDSynth.balanceOf(FEE_ADDRESS),
				...
				assert.deepEqual(
					await hUSDSynth.balanceOf(short.address),
				...
				assert.deepEqual(
					await hUSDSynth.balanceOf(account1),
					beforeUserBalance,
		...
		describe('Closing shorts', () => {
			...
			const accrueInterest = async () => {
				...
				// open another short to set a long/short skew
				await short.open(husdCollateral, ethAmountToShort, sETH, { from: account1 });
			...
			it('should repay with collateral and close the loan', async () => {
				...
				assert.bnEqual(await hUSDSynth.balanceOf(account1), toUnit(100));
			...
			it('should repay with collateral and close the loan considering interest accrued', async () => {
				...
				assert.bnEqual(await hUSDSynth.balanceOf(account1), beforeUserBalance);
		...
		describe('Drawing shorts', async () => {
			...
			it('should transfer the proceeds to the user', async () => {
				assert.bnEqual(await hUSDSynth.balanceOf(account1), toUnit(600));
			});
		...
		describe('Withdrawing shorts', async () => {
			...
			beforeEach(async () => {
				...
				previousBalance = await hUSDSynth.balanceOf(account1);
			...
			it('should transfer the withdrawn collateral to the user', async () => {
				assert.bnEqual(await hUSDSynth.balanceOf(account1), toUnit(100).add(previousBalance));
			});
		...
		describe('Closing shorts', async () => {
			...
			it('if the eth price goes down, the shorter makes profit', async () => {
				...
				await hUSDSynth.transfer(owner, toUnit(50), { from: account1 });
				...
				assert.bnEqual(await hUSDSynth.balanceOf(account1), toUnit(1050));
			});
			...
			it('if the eth price goes up, the shorter makes a loss', async () => {
				...
				await hUSDSynth.transfer(owner, toUnit(150), { from: account1 });
				await issue(sETHSynth, oneETH, account1);
				...
				assert.bnEqual(await hUSDSynth.balanceOf(account1), toUnit(950));
			});
		});
		...
		describe('Liquidating shorts', async () => {
			...
			const getExpectedValues = async ({
				...
				const liquidatedLoan = await exchangeRates.effectiveValue(hUSD, liquidatedAmounthUSD, sETH);
			...
			beforeEach(async () => {
				...
				tx = await short.open(husdCollateral, initialLoan, sETH, { from: account1 });
		...
		describe('System debt', async () => {
			...
			it('If there is 1 ETH and 1 short ETH, then the system debt is constant before and after a price change', async () => {
				...
				// simulate buying sETH for 150 husd.
				await hUSDSynth.burn(account1, toUnit(150));
				await issue(sETHSynth, oneETH, account1);
			...
			it('If there is 1 ETH and 2 short ETH, then the system debt decreases if the price goes up', async () => {
				...
				await hUSDSynth.burn(account1, toUnit(300));
				...
				// shorter has made 50 hUSD loss
				assert.bnEqual(await hUSDSynth.balanceOf(account1), toUnit(900));
			});
			...
			it('If there is 1 ETH and 2 short ETH, then the system debt increases if the price goes down', async () => {
				...
				await hUSDSynth.burn(account1, toUnit(100));
				await issue(sETHSynth, twoETH, account1);
				...
				assert.bnEqual(await hUSDSynth.balanceOf(account1), toUnit(1100));
			});
		});
		...
		describe('Determining the skew and interest rate', async () => {
			beforeEach(async () => {
				...
				await short.open(husdCollateral, oneBTC, sBTC, { from: account1 });
			});
