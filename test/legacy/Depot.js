...
contract('Depot', async accounts => {
	...
	describe('Ensure user can exchange ETH for Synths where the amount', async () => {
		...
		describe('exchangeSynthsForHAKAAtRate', () => {
			...
			beforeEach(async () => {
				...
				await synthetix.transfer(depot.address, depotHAKAAmount, {
					from: owner,
				});

				await synth.approve(depot.address, synthsToSend, fromPurchaser);

				const depotHAKABalance = await synthetix.balanceOf(depot.address);
	...
	describe('Ensure user can exchange ETH for HAKA', async () => {
		...
		it('ensure user get the correct amount of HAKA after sending ETH', async () => {
			...
			const purchaserHAKAStartBalance = await synthetix.balanceOf(purchaser);
			...
			const purchaseValueInSynthetix = divideDecimal(purchaseValueInSynths, hakaRate);

			const purchaserHAKAEndBalance = await synthetix.balanceOf(purchaser);

			// Purchaser HAKA balance should be equal to the purchase value we calculated above
			assert.bnEqual(purchaserHAKAEndBalance, purchaseValueInSynthetix);
		});
	});
	...
	describe('Ensure user can exchange Synths for Synthetix', async () => {
		...
		it('ensure user gets the correct amount of HAKA after sending 10 hUSD', async () => {
			...
			const purchaseValueInSynthetix = divideDecimal(synthsToSend, hakaRate);
	...
	describe('withdrawSynthetix', () => {
		...
		beforeEach(async () => {
			// Send some HAKA to the Depot contract
			await synthetix.transfer(depot.address, hakaAmount, {
				from: owner,
			});
		});
		...
		it('when owner calls withdrawSynthetix then withdrawSynthetix', async () => {
			const depotHAKABalanceBefore = await synthetix.balanceOf(depot.address);

			assert.bnEqual(depotHAKABalanceBefore, hakaAmount);

			await depot.withdrawSynthetix(hakaAmount, { from: owner });

			const depotHAKABalanceAfter = await synthetix.balanceOf(depot.address);
