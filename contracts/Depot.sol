...
contract Depot is Owned, Pausable, ReentrancyGuard, MixinResolver, IDepot {
        ...
        for (uint i = depositStartIndex; remainingToFulfill > 0 && i < depositEndIndex; i++) {
            ...
            if (deposit.user == address(0)) {
                ...
                if (deposit.amount > remainingToFulfill) {
                    ...
                    synthhUSD().transfer(msg.sender, remainingToFulfill);
                    ...
                    //       we request a specific transfer here, the fee is
                    //       automatically deducted and sent to the fee pool.
                    synthhUSD().transfer(msg.sender, deposit.amount);
    ...
    function _exchangeEtherForHAKA() internal returns (uint) {
        ...
        emit Exchange("ETH", msg.value, "HAKA", synthetixToSend);
     ...
     */
    function exchangeEtherForHAKAAtRate(uint guaranteedEtherRate, uint guaranteedSynthetixRate)
        external
        payable
        rateNotInvalid(HAKA)
        rateNotInvalid(ETH)
        ...
        )
    {
        ...
        require(
            guaranteedSynthetixRate == exchangeRates().rateForCurrency(HAKA),
    ...
    function _exchangeSynthsForHAKA(uint synthAmount) internal returns (uint) {
        ...
        synthhUSD().transferFrom(msg.sender, fundsWallet, synthAmount);

        // And send them the HAKA.
        synthetix().transfer(msg.sender, synthetixToSend);

        emit Exchange("hUSD", synthAmount, "HAKA", synthetixToSend);
    ...
    function withdrawMyDepositedSynths() external {
        ...
        synthhUSD().transfer(msg.sender, synthsToSend);
    ...
    function depositSynths(uint amount) external {
        ...
        synthhUSD().transferFrom(msg.sender, address(this), amount);
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[0] = CONTRACT_SYNTHHUSD;
    ...
    function synthhUSD() internal view returns (IERC20) {
