...
contract Wrapper is Owned, Pausable, MixinResolver, MixinSystemSettings, IWrapper {
    ...
    bytes32 private constant CONTRACT_SYNTH_HUSD = "SynthhUSD";
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[0] = CONTRACT_SYNTH_HUSD;
    ...
    function synthhUSD() internal view returns (ISynth) {
        return ISynth(requireAndGetAddress(CONTRACT_SYNTH_HUSD));
    }
    ...
    function _mint(uint amount) internal {
        ...
        if (excessAmountUsd > 0) {
            synthhUSD().issue(address(wrapperFactory()), excessAmountUsd);
        }
