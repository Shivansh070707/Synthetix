...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[4] = CONTRACT_SYNTHHUSD;
    ...
    function _synthhUSD() internal view returns (ISynth) {
        return ISynth(requireAndGetAddress(CONTRACT_SYNTHHUSD));
    }
    ...
    function _open(
        ...
        if (short) {
            _synthhUSD().issue(msg.sender, _exchangeRates().effectiveValue(currency, loanAmountMinusFee, hUSD));
    ...
    function _draw(uint id, uint amount) internal rateIsValid issuanceIsActive returns (uint, uint) {
        ...
        if (loan.short) {
            ...
            _synthhUSD().issue(msg.sender, _exchangeRates().effectiveValue(loan.currency, amountMinusFee, hUSD));
    ...
    function _payFees(uint amount, bytes32 synth) internal {
        if (amount > 0) {
            ...
            _synthhUSD().issue(_feePool().FEE_ADDRESS(), amount);
