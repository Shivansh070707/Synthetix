...
contract SynthRedeemer is ISynthRedeemer, MixinResolver {
    ...
    bytes32 private constant CONTRACT_SYNTHHUSD = "SynthhUSD";
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[1] = CONTRACT_SYNTHHUSD;
    }
    ...
    function hUSD() internal view returns (IERC20) {
        return IERC20(requireAndGetAddress(CONTRACT_SYNTHHUSD));
    }

    function totalSupply(IERC20 synthProxy) public view returns (uint supplyInhUSD) {
        supplyInhUSD = synthProxy.totalSupply().multiplyDecimal(redemptions[address(synthProxy)]);
    }

    function balanceOf(IERC20 synthProxy, address account) external view returns (uint balanceInhUSD) {
        balanceInhUSD = synthProxy.balanceOf(account).multiplyDecimal(redemptions[address(synthProxy)]);
    }
    ...
    function _redeem(IERC20 synthProxy, uint amountOfSynth) internal {
        ...
        uint amountInhUSD = amountOfSynth.multiplyDecimal(rateToRedeem);
        hUSD().transfer(msg.sender, amountInhUSD);
        emit SynthRedeemed(address(synthProxy), msg.sender, amountOfSynth, amountInhUSD);
    }
    ...
    function deprecate(IERC20 synthProxy, uint rateToRedeem) external onlyIssuer {
        ...
        uint supplyInhUSD = totalSynthSupply.multiplyDecimal(rateToRedeem);
        require(hUSD().balanceOf(address(this)) >= supplyInhUSD, "hUSD must first be supplied");
        redemptions[synthProxyAddress] = rateToRedeem;
        emit SynthDeprecated(address(synthProxy), rateToRedeem, totalSynthSupply, supplyInhUSD);
    }
    ...
    event SynthRedeemed(address synth, address account, uint amountOfSynth, uint amountInhUSD);
    event SynthDeprecated(address synth, uint rateToRedeem, uint totalSynthSupply, uint supplyInhUSD);
}
