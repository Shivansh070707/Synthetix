    ...
    function _exchangeAtomically(
...

        {
            ...
            if (sourceCurrencyKey == hUSD) {
                ...
                sourceHusdValue = systemConvertedAmount;
            } else {
                // Otherwise, convert source to hUSD value
                (uint amountReceivedInUSD, uint hUSDFee, , , , ) =
                    _getAmountsForAtomicExchangeMinusFees(
                        sourceAmountAfterSettlement,
                        sourceSettings,
                        usdSettings,
                ...
                sourceHusdValue = amountReceivedInUSD.add(hUSDFee);
            }

            // Check and update atomic volume limit
            _checkAndUpdateAtomicVolume(sourceSettings, sourceHusdValue);
        }
    ...
    function _checkAndUpdateAtomicVolume(
        ...
        uint currentVolume =
            uint(lastAtomicVolume.time) == block.timestamp
                ? uint(lastAtomicVolume.volume).add(sourceHusdValue)
                : sourceHusdValue;
